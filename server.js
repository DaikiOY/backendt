...
// Nuevas funciones para manejar la tabla `rented`
app.post('/api/rented', (req, res) => {
  const { name, price, url } = req.body;
  ...
  db.query('INSERT INTO rented (name, price, url) VALUES (?, ?, ?)', [name, price, url], (err) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    res.status(201).json({ message: 'Auto añadido a la tabla de rented exitosamente.' });
  });
});

app.get('/api/rented', (req, res) => {
  db.query('SELECT * FROM rented', (err, results) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    res.json(results);
...
app.delete('/api/rented', (req, res) => {
  ...
  db.query('DELETE FROM rented WHERE name = ?', [name], (err) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    res.status(200).json({ message: 'Auto eliminado de la tabla de rented exitosamente.' });
  });
});
